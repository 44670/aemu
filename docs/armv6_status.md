...
- Basic cumulative FPSCR exception flags for VFPv2 divide-by-zero, invalid
  ...
  arithmetic overflow; `OFC`/`UFC` for overflowing or underflowing
...
- Full VFP/VFPv2 is not implemented; FPSCR exception flags remain incomplete
  ...
  double-precision arithmetic underflow/general inexact behavior and less
  common arithmetic/conversion edge cases still need an
