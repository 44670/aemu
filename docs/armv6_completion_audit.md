...
Result: passing for the current in-crate unit test suite. The old external
process oracle integration tests were removed with the qemu backend.
...
| Test with functions | Unit tests in `src/armv6.rs`, probe tests in `src/elf_probe.rs` and `src/zip_probe.rs`, plus native-runtime smoke tests. | Partially complete; current tests pass but are representative rather than exhaustive/randomized. |
...
- VFP compare now sets cumulative `IOC` for signaling-NaN `VCMP` and any-NaN
  `VCMPE`, with unit and QEMU oracle coverage
...
- VFPv2 float-to-integer conversions now set cumulative `IOC` for NaN,
  out-of-range, and negative unsigned conversion cases, and saturate or zero
  results to match the QEMU oracle
...
- VFPv2 double-precision arithmetic now sets cumulative `IOC` for selected
  invalid add, multiply, divide, square-root, and multiply-accumulate cases,
  with direct unit and QEMU oracle coverage
- VFPv2 double-precision arithmetic now sets cumulative `OFC|IXC` for selected
  overflow add, multiply, divide, and multiply-accumulate cases, with direct
  unit and QEMU oracle coverage
- VFPv2 double-precision arithmetic now sets cumulative `UFC|IXC` for selected
  zero-result underflow multiply, divide, and multiply-accumulate cases, while
  exact subnormal multiplication remains flag-clean, with direct unit and QEMU
  oracle coverage
- VFPv2 double-precision add/sub now sets cumulative `IXC` for selected cases
  where a nonzero operand is rounded away, with direct unit and QEMU oracle
  coverage
- VFPv2 double-precision multiply now checks exactness with integer
...
- General coprocessor instructions are not implemented beyond VFP and CP15
  TLS/barrier shims.
- ELF/native execution is not complete yet: Java/native activity entrypoint
