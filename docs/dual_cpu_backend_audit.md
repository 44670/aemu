...
| QEMU ARMv7-A TCG executable backend | No runtime backend executes guest Android code through QEMU TCG yet. Existing QEMU use is in `tests/qemu_oracle.rs` as instruction-level subprocess oracles. | Missing |
| Bring MCPE up with QEMU backend | No `run-apk-native --cpu-backend qemu-armv7a-tcg` success path exists. | Missing |
| Compare QEMU and AEMU runtime behavior | Existing QEMU oracle tests compare selected instructions; there is no native-runtime trace/state comparator between the two backend modes. | Missing |
