...
fn neon_3diff_instr(u: bool, size: u32, vn: usize, vd: usize, opcode: u32, vm: usize) -> u32 {
    0xf280_0000
        | (u32::from(u) << 24)
        | (((vd >> 4) as u32 & 1) << 22)
        | ((size & 0x3) << 20)
...
#[test]
fn qemu_oracle_normal_store_clears_overlapping_exclusive_monitor() {
    let mut body = String::from(
        ".arch armv6k\n\
         ldr r4, =data\n\
         ldr r1, =0x11111111\n\
    ...
    let base = 0x5040;
    let mut folded = 0;
    let fold = |folded: &mut u32, value: u32| *folded ^= byte_fold(value);

    mem.store32(base, 0x1111_1111).unwrap();
    cpu.set_reg(4, base);
    ...
    fold(&mut folded, cpu.reg(0));
    fold(&mut folded, mem.load32(base).unwrap());

    mem.store32(base + 8, 0x1111_1111).unwrap();
    mem.store32(base + 12, 0x4444_4444).unwrap();
    cpu.set_reg(4, base + 8);
    cpu.execute_arm(0xe194_0f9f, 0, &mut mem).unwrap(); // ldrex r0, [r4]
    cpu.set_reg(2, 0x2222_2222);
    ...
    cpu.execute_arm(0xe184_0f93, 0, &mut mem).unwrap(); // strex r0, r3, [r4]
    fold(&mut folded, cpu.reg(0));
    fold(&mut folded, mem.load32(base + 8).unwrap());
    fold(&mut folded, mem.load32(base + 12).unwrap());

    mem.store16(base + 16, 0x1111).unwrap();
    cpu.set_reg(4, base + 16);
    cpu.execute_arm(0xe1f4_0f9f, 0, &mut mem).unwrap(); // ldrexh r0, [r4]
    cpu.set_reg(2, 0x22);
    ...
    fold(&mut folded, cpu.reg(0));
    fold(&mut folded, u32::from(mem.load16(base + 16).unwrap()));
...
#[test]
fn qemu_oracle_arm_blx_register_interworking_matches_interpreter() {
    let asm = oracle_program(
        "mov r4, #0\n\
         adr r1, thumb_target\n\
         add r1, r1, #1\n\
    ...
    cpu.set_reg(4, 0);
    cpu.execute_arm(0xe12f_ff31, 0x1000, &mut mem).unwrap(); // blx r1
    cpu.set_reg(0, 0x4d);
    cpu.execute_thumb(0x4770, 0x2002, &mut mem).unwrap(); // bx lr
    cpu.execute_arm(0xe024_4000, 0x1004, &mut mem).unwrap(); // eor r4, r4, r0
    cpu.execute_arm(0xe1a0_0004, 0x1008, &mut mem).unwrap(); // mov r0, r4

    assert_eq!(qemu_exit as u32, cpu.reg(0) & 0xff);
}

#[test]
fn qemu_oracle_arm_bx_register_interworking_matches_interpreter() {
    let asm = oracle_program(
        "mov r4, #0\n\
         adr r1, thumb_target\n\
         add r1, r1, #1\n\
    ...
    let mut mem = VecMemory::new(0, 4);
    cpu.set_reg(1, 0x2001);
    cpu.set_reg(2, 0x1004);
    cpu.set_reg(4, 0);
    cpu.execute_arm(0xe12f_ff11, 0x1000, &mut mem).unwrap(); // bx r1
    cpu.set_reg(0, 0x2a);
    cpu.execute_thumb(0x4710, 0x2002, &mut mem).unwrap(); // bx r2
    cpu.execute_arm(0xe024_4000, 0x1004, &mut mem).unwrap(); // eor r4, r4, r0
    cpu.execute_arm(0xe1a0_0004, 0x1008, &mut mem).unwrap(); // mov r0, r4
...
#[test]
fn qemu_oracle_stm_writeback_base_in_list_stores_old_base_matches_interpreter() {
    let asm = ".syntax unified\n\
         .arch armv6\n\
         .text\n\
         .arm\n\
    ...
    cpu.set_reg(0, 0xaa);
    cpu.set_reg(1, 0x3000);
    cpu.execute_arm(0xe8a1_0003, 0, &mut mem).unwrap(); // stmia r1!, {r0, r1}

    assert_eq!(mem.load32(0x3000).unwrap(), 0xaa);
    assert_eq!(mem.load32(0x3004).unwrap(), 0x3000);
    assert_eq!(cpu.reg(1), 0x3008);

    let folded = mem.load32(0x3000).unwrap()
        ^ 0xaa
        ^ mem.load32(0x3004).unwrap()
        ^ 0x3000
        ^ cpu.reg(1)
        ^ 0x3008;
...
fn qemu_oracle_thumb_blx_immediate_interworking_matches_interpreter() {
    ...
    cpu.set_isa(aemu::armv6::Isa::Thumb);
    cpu.set_reg(4, 0);
    cpu.execute_thumb(0xf000, 0x2000, &mut mem).unwrap(); // BLX prefix
    cpu.execute_thumb(0xe808, 0x2002, &mut mem).unwrap(); // BLX suffix to ARM
    cpu.set_reg(0, 0x5a);
    cpu.execute_arm(0xe12f_ff1e, 0x2018, &mut mem).unwrap(); // bx lr
    cpu.execute_thumb(0x4044, 0x2004, &mut mem).unwrap(); // eors r4, r0
    cpu.set_reg(0, cpu.reg(4));

    assert_eq!(qemu_exit as u32, cpu.reg(0) & 0xff);
}

#[test]
fn qemu_oracle_thumb_blx_register_interworking_matches_interpreter() {
    let asm = ".syntax unified\n\
         .arch armv6\n\
         .text\n\
         .arm\n\
    ...
    cpu.set_reg(4, 0);
    cpu.execute_thumb(0x4780, 0x1000, &mut mem).unwrap(); // blx r0
    cpu.set_reg(0, 0x55);
    cpu.execute_arm(0xe12f_ff1e, 0x2004, &mut mem).unwrap(); // bx lr
    cpu.execute_thumb(0x4044, 0x1002, &mut mem).unwrap(); // eors r4, r0
    cpu.set_reg(0, cpu.reg(4));
...
#[test]
fn qemu_oracle_thumb_bx_register_interworking_matches_interpreter() {
    let asm = ".syntax unified\n\
         .arch armv6\n\
         .text\n\
         .arm\n\
    ...
    cpu.set_isa(aemu::armv6::Isa::Thumb);
    cpu.set_reg(0, 0x2000);
    cpu.execute_thumb(0x4700, 0x1000, &mut mem).unwrap(); // bx r0
    assert!(!cpu.cpsr.t);
    assert_eq!(cpu.pc(), 0x2000);
    cpu.execute_arm(0xe3a0_0066, 0x2000, &mut mem).unwrap(); // mov r0, #0x66
...
fn qemu_oracle_thumb_bl_immediate_matches_interpreter() {
    ...
    cpu.set_isa(aemu::armv6::Isa::Thumb);
    cpu.set_reg(4, 0);
    cpu.execute_thumb(0xf000, 0x2000, &mut mem).unwrap(); // BL prefix
    cpu.execute_thumb(0xf806, 0x2002, &mut mem).unwrap(); // BL suffix to Thumb
    cpu.set_reg(0, 0x6b);
    cpu.execute_thumb(0x4770, 0x2012, &mut mem).unwrap(); // bx lr
    cpu.execute_thumb(0x4044, 0x2004, &mut mem).unwrap(); // eors r4, r0
    cpu.set_reg(0, cpu.reg(4));
...
#[test]
fn qemu_oracle_neon_saturating_add_sub_64_matches_interpreter() {
    let asm = neon_oracle_program(
        "ldr r5, =values_qadd_s64\n\
         vld1.64 {d1, d2}, [r5]\n\
         vqadd.s64 d0, d1, d2\n\
         ldr r5, =values_qadd_u64\n\
         vld1.64 {d3, d4}, [r5]\n\
         vqadd.u64 d5, d3, d4\n\
         ldr r5, =values_qsub_u64\n\
         vld1.64 {d6, d7}, [r5]\n\
         vqsub.u64 d8, d6, d7\n\
         ldr r5, =out_qadd64\n\
         vst1.64 {d0}, [r5]\n\
         ...
         .pushsection .data\n\
         .align 3\n\
         values_qadd_s64: .quad 0x7ffffffffffffffe, 2\n\
         values_qadd_u64: .quad 0xfffffffffffffffe, 5\n\
         values_qsub_u64: .quad 1, 3\n\
         out_qadd64: .space 24\n\
         .popsection\n",
    );
    let Some(qemu_exit) = run_armv7_neon_linux_exit(&asm) else {
    ...
    cpu.set_dreg(2, 2);
    cpu.execute_arm(
        neon_3same_instr(false, 3, 1, 0, 0, false, true, 2),
        0,
        &mut mem,
    )
    ...
    cpu.set_dreg(4, 5);
    cpu.execute_arm(
        neon_3same_instr(true, 3, 3, 5, 0, false, true, 4),
        0,
        &mut mem,
    )
    ...
    cpu.set_dreg(7, 3);
    cpu.execute_arm(
        neon_3same_instr(true, 3, 6, 8, 2, false, true, 7),
        0,
        &mut mem,
    )
    .unwrap(); // vqsub.u64 d8, d6, d7

    let folded = (cpu.dreg(0) as u32)
        .wrapping_add((cpu.dreg(0) >> 32) as u32)
        .wrapping_add(cpu.dreg(5) as u32)
        .wrapping_add((cpu.dreg(5) >> 32) as u32)
        .wrapping_add(cpu.dreg(8) as u32)
        .wrapping_add((cpu.dreg(8) >> 32) as u32);
    assert_eq!(qemu_exit as u32, folded & 0xff);
}

#[test]
fn qemu_oracle_neon_mcpe_bitwise_select_matches_interpreter() {
    let asm = neon_oracle_program(
        "ldr r5, =values_logic\n\
         vld1.64 {d0, d1, d2, d3}, [r5]!\n\
         vld1.64 {d5}, [r5]\n\
         vorn d4, d2, d3\n\
         vbsl d0, d2, d3\n\
         vbit d1, d2, d3\n\
         ...
         .pushsection .data\n\
         .align 3\n\
         values_logic: .quad 0x00ff00ff55aa55aa, 0xf0f0f0f00f0f0f0f\n\
                       .quad 0x1122334455667788, 0x8877665544332211\n\
                       .quad 0x0f0f0f0ff0f0f0f0\n\
         out_logic: .space 32\n\
         .popsection\n",
    );
    let Some(qemu_exit) = run_armv7_neon_linux_exit(&asm) else {
    ...
    let mut cpu = Cpu::new();
    let mut mem = VecMemory::new(0, 4);
    cpu.set_dreg(0, 0x00ff_00ff_55aa_55aa);
    cpu.set_dreg(1, 0xf0f0_f0f0_0f0f_0f0f);
    cpu.set_dreg(2, 0x1122_3344_5566_7788);
    cpu.set_dreg(3, 0x8877_6655_4433_2211);
    cpu.set_dreg(5, 0x0f0f_0f0f_f0f0_f0f0);
    cpu.execute_arm(
        neon_3same_instr(false, 3, 2, 4, 1, false, true, 3),
        0,
        &mut mem,
    )
    .unwrap(); // vorn d4, d2, d3
    cpu.execute_arm(
        neon_3same_instr(true, 1, 2, 0, 1, false, true, 3),
        0,
        &mut mem,
    )
    .unwrap(); // vbsl d0, d2, d3
    cpu.execute_arm(
        neon_3same_instr(true, 2, 2, 1, 1, false, true, 3),
        0,
        &mut mem,
    )
    .unwrap(); // vbit d1, d2, d3
    cpu.execute_arm(
        neon_3same_instr(true, 3, 2, 5, 1, false, true, 3),
        0,
        &mut mem,
    )
    .unwrap(); // vbif d5, d2, d3

    let folded = (cpu.dreg(0) as u32)
        ^ ((cpu.dreg(0) >> 32) as u32)
        ^ (cpu.dreg(1) as u32)
...
#[test]
fn qemu_oracle_neon_mcpe_widening_accumulate_matches_interpreter() {
    let asm = neon_oracle_program(
        "ldr r5, =values_wide\n\
         vld1.16 {d0, d1}, [r5]!\n\
         vld1.8 {d4}, [r5]!\n\
         ...
         .pushsection .data\n\
         .align 3\n\
         values_wide: .hword 0xff9c, 0x00c8, 0x7fff, 0x8000, 0x0000, 0x0100, 0xff00, 0x1234\n\
                      .byte 1, 0xff, 0x7f, 0x80, 2, 3, 0xfe, 0x40\n\
                      .hword 0x0100, 0x0005, 0x00ff, 0x8000, 0xffff, 0x4000, 0x0001, 0x1000\n\
                      .byte 0xff, 1, 0x80, 0x7f, 2, 0xfe, 3, 4\n\
    ...
    cpu.set_dreg(
        0,
        u64::from_le_bytes([0x9c, 0xff, 0xc8, 0x00, 0xff, 0x7f, 0x00, 0x80]),
    );
    cpu.set_dreg(1, u64::from_le_bytes([0, 0, 0, 1, 0, 0xff, 0x34, 0x12]));
    cpu.set_dreg(
        4,
        u64::from_le_bytes([1, 0xff, 0x7f, 0x80, 2, 3, 0xfe, 0x40]),
    );
    cpu.execute_arm(neon_3diff_instr(false, 0, 0, 0, 1, 4), 0, &mut mem)
        .unwrap(); // vaddw.s8 q0, q0, d4

    cpu.set_dreg(2, u64::from_le_bytes([0, 1, 5, 0, 0xff, 0, 0, 0x80]));
    cpu.set_dreg(3, u64::from_le_bytes([0xff, 0xff, 0, 0x40, 1, 0, 0, 0x10]));
    cpu.set_dreg(5, u64::from_le_bytes([0xff, 1, 0x80, 0x7f, 2, 0xfe, 3, 4]));
    cpu.execute_arm(neon_3diff_instr(true, 0, 2, 2, 3, 5), 0, &mut mem)
        .unwrap(); // vsubw.u8 q1, q1, d5

    cpu.set_dreg(6, u64::from(1000u32) | (u64::from(2000u32) << 32));
    cpu.set_dreg(7, u64::from(3000u32) | (u64::from(4000u32) << 32));
    cpu.set_dreg(8, 0x0006_0005_0004_0003);
    cpu.set_dreg(9, 0x000a_0009_0008_0007);
    cpu.execute_arm(neon_3diff_instr(true, 1, 8, 6, 10, 9), 0, &mut mem)
        .unwrap(); // vmlsl.u16 q3, d8, d9
...
#[test]
fn qemu_oracle_neon_mcpe_narrow_and_estimate_matches_interpreter() {
    let asm = neon_oracle_program(
        "ldr r5, =values_narrow\n\
         vld1.64 {d2, d3}, [r5]!\n\
         vqshrn.u64 d0, q1, #1\n\
         vld1.64 {d4, d5}, [r5]!\n\
         vqrshrn.u64 d1, q2, #1\n\
         ...
         .pushsection .data\n\
         .align 3\n\
         values_narrow: .quad 0x00000001ffffffff, 0xffffffffffffffff\n\
                        .quad 0x00000001ffffffff, 0x0000000000000003\n\
                        .word 0x40000000, 0x40800000\n\
                        .word 0x3f000000, 0x3f800000\n\
         out_narrow: .space 24\n\
         .popsection\n",
    );
    let Some(qemu_exit) = run_armv7_neon_linux_exit(&asm) else {
    ...
    let mut cpu = Cpu::new();
    let mut mem = VecMemory::new(0, 4);
    cpu.set_dreg(2, 0x0000_0001_ffff_ffff);
    cpu.set_dreg(3, u64::MAX);
    cpu.execute_arm(
        neon_2reg_shift_instr(true, 63, 0, 9, false, false, 2),
        0,
        &mut mem,
    )
    .unwrap(); // vqshrn.u64 d0, q1, #1

    cpu.set_dreg(4, 0x0000_0001_ffff_ffff);
    cpu.set_dreg(5, 3);
    cpu.execute_arm(
        neon_2reg_shift_instr(true, 63, 1, 9, false, true, 4),
        0,
        &mut mem,
    )
    .unwrap(); // vqrshrn.u64 d1, q2, #1
    ...
    cpu.set_dreg(
        7,
        u64::from(0.5f32.to_bits()) | (u64::from(1.0f32.to_bits()) << 32),
    );
    cpu.execute_arm(
        neon_3same_instr(false, 2, 6, 8, 15, false, true, 7),
        0,
        &mut mem,
    )
    .unwrap(); // vrsqrts.f32 d8, d6, d7
...
#[test]
fn qemu_oracle_neon_mcpe_high_narrow_and_recip_step_matches_interpreter() {
    let asm = neon_oracle_program(
        "ldr r5, =values_high_narrow\n\
         vld1.64 {d0, d1}, [r5]!\n\
         vld1.64 {d2, d3}, [r5]!\n\
         vaddhn.i16 d8, q0, q1\n\
         vsubhn.i16 d9, q0, q1\n\
         vraddhn.i16 d10, q0, q1\n\
         vrsubhn.i16 d11, q0, q1\n\
         vld1.32 {d12, d13}, [r5]!\n\
         vld1.32 {d14, d15}, [r5]\n\
         vrecps.f32 q8, q6, q7\n\
         ldr r5, =out_high_narrow\n\
         vst1.64 {d8, d9, d10, d11}, [r5]!\n\
         vst1.64 {d16, d17}, [r5]\n\
         ldr r5, =out_high_narrow\n\
         mov r0, #0\n\
         .rept 12\n\
         ...
         .pushsection .data\n\
         .align 3\n\
         values_high_narrow: .hword 0x0100, 0xff00, 0x7f00, 0x8000\n\
                             .hword 0x1234, 0xfedc, 0x0001, 0xffff\n\
                             .hword 0x0010, 0x0100, 0x0100, 0x8000\n\
                             .hword 0x1111, 0x2222, 0xffff, 0x0001\n\
                             .word 0x3f000000, 0x40000000, 0xbf800000, 0x40800000\n\
                             .word 0x3f800000, 0x3e800000, 0xc0000000, 0x3f000000\n\
         out_high_narrow: .space 48\n\
         .popsection\n",
    );
    let Some(qemu_exit) = run_armv7_neon_linux_exit(&asm) else {
        return;
    ...
    cpu.set_dreg(
        0,
        u64::from_le_bytes([0x00, 0x01, 0x00, 0xff, 0x00, 0x7f, 0x00, 0x80]),
    );
    cpu.set_dreg(
        1,
        u64::from_le_bytes([0x34, 0x12, 0xdc, 0xfe, 0x01, 0x00, 0xff, 0xff]),
    );
    cpu.set_dreg(
        2,
        u64::from_le_bytes([0x10, 0x00, 0x00, 0x01, 0x00, 0x01, 0x00, 0x80]),
    );
    cpu.set_dreg(
        3,
        u64::from_le_bytes([0x11, 0x11, 0x22, 0x22, 0xff, 0xff, 0x01, 0x00]),
    );
    cpu.execute_arm(neon_3diff_instr(false, 0, 0, 8, 4, 2), 0, &mut mem)
        .unwrap(); // vaddhn.i16 d8, q0, q1
    cpu.execute_arm(neon_3diff_instr(false, 0, 0, 9, 6, 2), 0, &mut mem)
        .unwrap(); // vsubhn.i16 d9, q0, q1
    cpu.execute_arm(neon_3diff_instr(true, 0, 0, 10, 4, 2), 0, &mut mem)
        .unwrap(); // vraddhn.i16 d10, q0, q1
    cpu.execute_arm(neon_3diff_instr(true, 0, 0, 11, 6, 2), 0, &mut mem)
        .unwrap(); // vrsubhn.i16 d11, q0, q1

    let pack2 = |a: f32, b: f32| u64::from(a.to_bits()) | (u64::from(b.to_bits()) << 32);
    cpu.set_dreg(12, pack2(0.5, 2.0));
    cpu.set_dreg(13, pack2(-1.0, 4.0));
    cpu.set_dreg(14, pack2(1.0, 0.25));
    cpu.set_dreg(15, pack2(-2.0, 0.5));
    cpu.execute_arm(
        neon_3same_instr(false, 2, 12, 16, 15, true, true, 14),
        0,
        &mut mem,
    )
    .unwrap(); // vrecps.f32 q8, q6, q7
...
fn qemu_oracle_neon_mcpe_scalar_long_matches_interpreter() {
    let asm = neon_oracle_program(
        "ldr r5, =values_mul_a\n\
         vld1.32 {d28}, [r5]\n\
         ldr r5, =values_scalar_a\n\
         vld1.32 {d0}, [r5]\n\
         .inst 0xf3accac0\n\
         ldr r5, =values_mul_b\n\
         vld1.32 {d29}, [r5]\n\
         ldr r5, =values_scalar_b\n\
         vld1.32 {d4}, [r5]\n\
         .inst 0xf3adc2c4\n\
         ldr r5, =out_scalar_long\n\
         vst1.64 {d12, d13}, [r5]\n\
         ldr r0, [r5]\n\
         ldr r1, [r5, #4]\n\
         eor r0, r0, r1\n\
         ...
         .pushsection .data\n\
         .align 3\n\
         values_mul_a: .word 3, 4\n\
         values_scalar_a: .word 5, 7\n\
         values_mul_b: .word 2, 3\n\
         values_scalar_b: .word 11, 13\n\
         out_scalar_long: .space 16\n\
         .popsection\n",
    );
    let Some(qemu_exit) = run_armv7_neon_linux_exit(&asm) else {
        return;
    ...
    let folded = (cpu.dreg(12) as u32)
        ^ ((cpu.dreg(12) >> 32) as u32)
        ^ (cpu.dreg(13) as u32)
        ^ ((cpu.dreg(13) >> 32) as u32);
    assert_eq!(qemu_exit as u32, folded & 0xff);
}
...
#[test]
fn qemu_oracle_vfp_basic_exception_flags_match_interpreter() {
    let asm = oracle_program(
        ".fpu vfp\n\
         mov r12, #0\n\
         mov r0, #0\n\
...
#[test]
fn qemu_oracle_vfp_double_invalid_exception_flags_match_interpreter() {
    let fold_reg = |body: &mut String, reg: &str| {
        body.push_str(&format!(
            "eor r12, r12, {reg}\n\
             eor r12, r12, {reg}, lsr #8\n\
    ...
    let mut mem = VecMemory::new(0, 4);
    let mut folded = 0;
    let fold_cpu_fpscr = |folded: &mut u32, fpscr: u32| {
        *folded = folded.wrapping_add(*folded << 5) ^ byte_fold(fpscr)
    };
    ...
    fold_cpu_fpscr(&mut folded, cpu.fpscr);
...
#[test]
fn qemu_oracle_vfp_double_overflow_exception_flags_match_interpreter() {
    let fold_reg = |body: &mut String, reg: &str| {
        body.push_str(&format!(
            "eor r12, r12, {reg}\n\
             eor r12, r12, {reg}, lsr #8\n\
    ...
    let mut mem = VecMemory::new(0, 4);
    let mut folded = 0;
    let fold_result = |folded: &mut u32, result: u64, fpscr: u32| {
        *folded = folded.wrapping_add(*folded << 5) ^ double_byte_fold(result) ^ byte_fold(fpscr)
    };

    cpu.set_dreg(0, f64::MAX.to_bits());
    cpu.set_dreg(1, f64::MAX.to_bits());
    cpu.execute_arm(0xee30_2b01, 0, &mut mem).unwrap(); // vadd.f64 d2, d0, d1
    fold_result(&mut folded, cpu.dreg(2), cpu.fpscr);
    ...
    cpu.execute_arm(0xee07_6b08, 0, &mut mem).unwrap(); // vmla.f64 d6, d7, d8
    fold_result(&mut folded, cpu.dreg(6), cpu.fpscr);
    cpu.set_reg(0, folded);
...
#[test]
fn qemu_oracle_vfp_double_absorbed_inexact_exception_flags_match_interpreter() {
    let fold_reg = |body: &mut String, reg: &str| {
        body.push_str(&format!(
            "eor r12, r12, {reg}\n\
             eor r12, r12, {reg}, lsr #8\n\
...
#[test]
fn qemu_oracle_vfp_double_multiply_inexact_exception_flags_match_interpreter() {
    let fold_reg = |body: &mut String, reg: &str| {
        body.push_str(&format!(
            "eor r12, r12, {reg}\n\
             eor r12, r12, {reg}, lsr #8\n\
...
#[test]
fn qemu_oracle_vfp_double_divide_inexact_exception_flags_match_interpreter() {
    let fold_reg = |body: &mut String, reg: &str| {
        body.push_str(&format!(
            "eor r12, r12, {reg}\n\
             eor r12, r12, {reg}, lsr #8\n\
...
#[test]
fn qemu_oracle_vfp_double_zero_underflow_exception_flags_match_interpreter() {
    let fold_reg = |body: &mut String, reg: &str| {
        body.push_str(&format!(
            "eor r12, r12, {reg}\n\
             eor r12, r12, {reg}, lsr #8\n\
...
#[test]
fn qemu_oracle_vfp_short_vector_arithmetic_matches_interpreter() {
    let mut body = String::from(
        ".fpu vfp\n\
         mov r12, #0\n\
         ldr r2, =0x00010000\n\
    ...
    fold_single(&mut body, "s10");
    fold_single(&mut body, "s11");
    body.push_str(
        "ldr r0, =0x00000000\n\
         ldr r1, =0x3ff80000\n\
         vmov d8, r0, r1\n\
    ...
    fold_double(&mut body, "d4");
    fold_double(&mut body, "d5");
    body.push_str("mov r0, r12");
    ...
    folded ^= byte_fold(cpu.sreg(10));
    folded ^= byte_fold(cpu.sreg(11));
...
#[test]
fn qemu_oracle_vfp_compare_conversion_stay_scalar_in_vector_mode() {
    let mut body = String::from(
        ".fpu vfp\n\
         mov r12, #0\n\
         ldr r2, =0x00010000\n\
    ...
    body.push_str(
        "ldr r2, =0x00410000\n\
         ...
         ldr r0, =0x40e80000\n\
         vmov s19, r0\n\
         vcvtr.u32.f32 s12, s18\n",
    );
    fold_single(&mut body, "s12");
    fold_single(&mut body, "s13");
    body.push_str(
        "ldr r0, =0xeeeeeeee\n\
         vmov s14, r0\n\
         ldr r0, =0xffffffff\n\
...
fn qemu_oracle_vfp_conversion_exception_flags_match_interpreter() {
    ...
    let fold_conversion = |body: &mut String| {
        body.push_str(
            "vmov r4, s1\n\
             vmrs r3, fpscr\n",
        );
...
#[test]
fn qemu_oracle_vfp_compare_exception_flags_match_interpreter() {
    let mut body = String::from(
        ".fpu vfp\n\
         mov r12, #0\n",
    );
